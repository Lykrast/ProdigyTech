...
public class ContainerFoodEnricher extends ContainerMachine<TileFoodEnricher> {
    private int temperature;
    private int temperatureOut;
    private int processTime;
    private int processTimeMax;
    
	public ContainerFoodEnricher(InventoryPlayer userInv, TileFoodEnricher tile) {
		super(tile);
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            {
            	if (TileFoodEnricher.isValidInput(itemstack1))
                {
