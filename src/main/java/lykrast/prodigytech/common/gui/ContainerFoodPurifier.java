...
public class ContainerFoodPurifier extends ContainerMachine<TileFoodPurifier> {
    private int temperature;
    private int temperatureOut;
    private int processTime;
    private int processTimeMax;
    
	public ContainerFoodPurifier(InventoryPlayer userInv, TileFoodPurifier tile) {
		super(tile);
    	...
    	addSlotToContainer(new Slot(tile, 0, 56, 35) {
			...
			public boolean isItemValid(ItemStack stack) {
                return TileFoodPurifier.isValidInput(stack);
            }
    	});
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            {
            	if (TileFoodPurifier.isValidInput(itemstack1))
                {
