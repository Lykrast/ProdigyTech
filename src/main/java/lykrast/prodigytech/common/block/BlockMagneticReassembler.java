...
public class BlockMagneticReassembler extends BlockHotAirMachine<TileMagneticReassembler> implements ICustomItemBlock {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        {
        	TileMagneticReassembler tile = getTileEntity(worldIn,pos);

            if (tile != null)
            {
                playerIn.openGui(ProdigyTech.instance, ProdigyTechGuiHandler.MAGNETIC_REASSEMBLER, worldIn, pos.getX(), pos.getY(), pos.getZ());
